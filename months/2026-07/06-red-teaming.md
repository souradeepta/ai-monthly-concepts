...
The repeated-call pattern is useful because it separates exploration from judgment. A discovery worker can ask many narrow questions about reachable code paths, but each candidate should carry a location, an input assumption, a proposed trigger, and an evidence pointer. The red-team harness then decides which candidates deserve validation. This is an important control boundary: an enthusiastic model can generate plausible vulnerability descriptions faster than a team can inspect them, so the queue must prioritize candidates by asset exposure and reproducibility rather than by confident language. The July source supports the use of repeated model calls for broader code exploration; the following separation of stages is an engineering inference for operating that capability safely.

Validation should run in an environment whose state can be reset. Pin the repository revision, dependency lockfile, compiler or interpreter version, feature flags, and test fixture. If a candidate requires a network response, replace the external service with a deterministic mock or record/replay fixture. Record whether the candidate caused a state change, not just whether a model-generated explanation sounded correct. A finding that reproduces only when an undocumented environment variable is set may still matter, but the condition belongs in the severity rationale and release gate. A finding that cannot reproduce after three controlled attempts should become an unverified hypothesis, not silently disappear from the queue.

The consolidated report described in the July release also suggests a useful reporting distinction. Many low-level observations may point to one root cause, such as an unchecked path reaching several call sites. Deduplicate them only after preserving the individual evidence paths. The parent finding can identify the common control gap, while child records retain the affected files, test cases, and regression coverage. This avoids both inflated severity from counting duplicates and under-reporting from collapsing distinct reachable assets into one vague ticket. The report should state what the model searched, what it did not search, and which observations a human confirmed.

For an AI application, apply the same discipline to agent trajectories. Store the fixture and trusted identity, the model proposal, normalized tool arguments, gateway decision, executor receipt, and terminal state as separate events. A refusal is not proof that the gateway is safe, and a gateway denial is not proof that the planner is safe; they answer different questions. Compare a baseline policy with the candidate policy across the same scenario set, then add a regression for every confirmed boundary failure. This lets a red-team run produce durable engineering value even when the model’s behavior changes after a routine update.

Finally, make closure a controlled state transition. `open` means a candidate exists; `reproduced` means the fixture demonstrates the behavior; `mitigated` means a control changed; `verified` means the original and neighboring cases pass; and `accepted-risk` means an accountable owner recorded why residual exposure remains. The states should include timestamps, affected release, and owner. This is not bureaucracy around the July source’s capability; it is the mechanism that prevents AI-assisted search from becoming an unowned stream of alarming prose. The red team measures whether protected assets became harder to reach, easier to detect, or faster to recover—not merely how many prompts or code paths were generated.
