...
The AWS release makes checkpointing a platform primitive rather than a convention in one worker process. Its custom Durable Execution SDK support and language-agnostic conformance tests indicate that a durable workflow can be exercised across implementation choices while retaining a defined execution contract. The source does not say that an AI planner becomes reliable automatically. The engineering implication is narrower: put the planner inside a runtime that can persist waits, retries, and progress, then make the product’s task state explicit on top of that runtime. A process restart should resume from a durable checkpoint, not from whatever text happened to remain in memory.

The robotics release supplies a different kind of evidence. Its product description connects multi-step planning with continuous-video progress tracking, self-correction, and human teleoperation. Those capabilities describe a physical agent that observes a changing environment; they do not establish a general durable-workflow implementation. For a software task, the corresponding state must be represented as evidence: the version of the input, the action proposal, the tool invocation identifier, the result or timeout, and the next permitted transition. A progress message without one of those durable anchors is presentation, not recovery state.

A long-running run should therefore have two clocks. The execution clock records when work was enqueued, started, checkpointed, retried, and completed. The authority clock records when credentials, approvals, leases, or user permissions expire. Replaying a step may be valid under the execution clock but invalid under the authority clock. Before resuming, revalidate the user, tenant, tool scope, model policy, and external object version. This is especially important after a weekend pause or a deployment that changed the action schema. Durable state preserves history; it does not grant stale authority.

Design every external effect around a recovery question. If the worker has an idempotency key and a receipt, advance. If it has an intent but no receipt, query the external system or move to `UNKNOWN_EFFECT`. If it has neither, retry only after checking that the action is still allowed. For a physical or financial effect, the safe choice may be operator review rather than automatic reconciliation. Make these branches visible in the run record so support can explain why a task waited instead of repeatedly issuing the same request.

Conformance tests should cover more than a happy-path completion. Crash at each checkpoint boundary, deliver a duplicate queue message, delay a tool result, revoke permission while waiting, and restart after a deployment. Assert that the durable history contains one logical effect, that an expired task cannot create a new effect, and that an operator can resume from a named state. This keeps the lesson focused on product-run semantics: lesson 19 addresses deterministic runtime replay and compatibility, while this lesson asks whether a user-facing task remains understandable, authorized, and recoverable over time.
