...
## Query rewriting without widening access

Query rewriting can improve recall by expanding abbreviations, resolving an entity, or generating a few alternate phrasings. It also creates a second authorization surface. A rewrite must inherit the original principal, tenant, purpose, and deadline; it must not turn “my invoices” into a global search for invoices or copy a sensitive phrase into a broader logging system. Keep the original question, generated rewrites, and accepted filters in the retrieval trace. Apply access and freshness constraints to every candidate produced by every rewrite, then merge results by source identity before reranking. A useful test gives the rewriter an ambiguous name, a cross-tenant identifier, and an instruction embedded in a document. The expected outcome is a narrowed or empty candidate set, never a broader corpus search. Measure recall gain against unauthorized-candidate rate, rewrite latency, and added context duplication.
